...
::: tip
当资源站为 **Mikan** 时，AB 会先搜索 Mikan 的番组目录，再读取番组页上的字幕组列表。结果中的每个标签对应一个字幕组；订阅时使用 Mikan 给出的精确番组 RSS，例如
`/RSS/Bangumi?bangumiId=3993&subgroupid=615`，不会再根据标签文字生成关键词搜索 RSS。

其他资源站仍使用各自的 RSS 搜索模板与配置的解析器。
:::
